...
namespace MightyXOR.Core.SecretSharing.Shamir
{
    ...
    public class Reconstructor
    {
        ...
        /// <summary>
        /// Reconstructs the secret given k shared files
        /// </summary>
        /// <param name="k">Number of files to use</param>
        /// <param name="isMightyXorFile">Is the secret a MightyXOR file?</param>
        /// <param name="outputDirectory">Output directory of the secret</param>
        public void Reconstruct(byte k, bool isMightyXorFile, string outputDirectory)
        {
            CheckParameters(k);
            ...
            try
            {
                ...
                if (isMightyXorFile)
                {
                    var mightyXorFile = new BsonSerializer().DeserializeObject<MightyXorFile>(secretBytes);
                    mightyXorFile?.PersistContent($"{outputDirectory}\\{mightyXorFile.Header?.Name}");
                }
                else
