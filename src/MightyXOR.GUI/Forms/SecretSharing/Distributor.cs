    ...
    private void DistributeBtn_Click(object? sender, EventArgs e)
    {
        try
        {
            ...
            if (useHeaderChk.Checked)
            {
                ...
                var fileHeader = new MightyXorFileHeader(new FileInfo(pathToSecret),
                                                            new HashUtil(HashType.MD5).GenerateHash(secretBytes));
                var mightyXorFile = new MightyXorFile(fileHeader, secretBytes);
                var distributor = new Core.SecretSharing.Shamir.Distributor(mightyXorFile);
